...
try {
    ...
    if (isset($_GET['id']) AND $_GET['id'] > 0) {
        ...
        if (!isset($_SESSION['id'])) 
            ...
            header('LOCATION:formulaire1.php?id='.$_SESSION['id']);
    }
    ...
    if (isset ($_POST['id_voiture'])) {
        $test = $pdo->prepare("SELECT id FROM voiture");
        $test->execute();
...
<header>
    ...
    <?php 
        ...
        else if (!isset($_SESSION['id_voiture'])) {
            if ($userinfo['admin'] == 0) {
                echo '<div id="erreur2">';
                echo '<br><h2> <font color="red">Veuillez choisir une voiture avant de continuer</font> </h2><br>';
...
<form role="form" class="bookform form-inline row" action="envoyer_formulaire1.php?id=<?php echo $_SESSION['id'] ?>" method="POST">
    <br>
    

        <label> Voiture choisie : </label>
        <?php
            if (isset($_SESSION['id_voiture'])) {
            ...
            while ($location = $affiche->fetch()) {
                echo $location['marque'].' '.$location['modele'];
        ?>
                <input type="hidden" name="id_voiture" value="<?php echo $_SESSION['id_voiture'] ?>" >
                <input type="hidden" name="disponnibilite" value="<?php echo $location['disponnibilite'] ?>" >
        <?php
            }
            $affiche->closeCursor();
        ?>
        ...
        if (isset($_SESSION['date1'])) {
            echo $_SESSION['date1'];
            echo '<input type="hidden" name="date1" value="'.$_SESSION['date1'].'">';
        }
        ?>
    ...
    <?php 
        if (isset($_GET['id']) AND $_GET['id'] > 0) {
            if (isset($_SESSION['id']) AND $userinfo['id'] == $_SESSION['id']) {
                    echo '<input type="reset" name="reset" value="Annuler"  onclick=\'location.href="transport.php?id='.$_GET['id'].'"\'>';
            }
        }
        else {
            echo '<input type="reset" name="reset" value="Annuler" onclick=\'location.href="transport.php"\'>';
        }
    ?>
